...
export default function HotelChat() {
  ...
  const fetchThreads = async () => {
    if (!buyerId) return;
    try {
      const res = await fetch(`/api/chat/threads?userId=${buyerId}`);
  ...
  const fetchMessages = async () => {
    if (!buyerId || !activeThread) return;
    try {
      const res = await fetch(`/api/chat?userId1=${buyerId}&userId2=${activeThread.otherUserId}`);
      const data = await res.json();
      if (data.success) {
  ...
  const text = {
    si: {
      title: "මගේ පණිවිඩ",
      subtitle: "ගොවීන් සමඟ සෘජුව සිදුකළ ගනුදෙනු කතාබස් මෙතැනින් බලන්න.",
      noThreads: "තවමත් කිසිදු පණිවිඩ සංවාදයක් ආරම්භ කර නැත.",
      selectPrompt: "කතාබස් කිරීමට ගොවි මහතෙකුගේ නමක් තෝරන්න.",
    ...
    en: {
      title: "My Messages",
      subtitle: "Coordinate direct purchase negotiations and deliveries with local farmers.",
      noThreads: "No conversations started yet.",
      selectPrompt: "Select a farmer from the sidebar to open the chat window.",
    ...
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button
          ...
          className="self-start sm:self-center bg-white hover:bg-gray-50 border border-gray-200 text-xs font-black px-4 py-2.5 rounded-xl transition cursor-pointer shadow-sm z-10"
        >
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-gray-400 font-medium bg-white rounded-3xl border border-dashed border-gray-200 max-w-2xl mx-auto p-8">
